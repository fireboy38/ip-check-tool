  ...
  return (
    <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-semibold text-white">全球延迟测试</h2>
      </div>
          ...
          {filtered.map((node, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-white">{node.city}</span>
                <span className="text-slate-500 text-sm">{node.country}</span>
              </div>
              <div className="flex items-center gap-4 text-sm">
                <span className={`font-mono font-medium ${getLatencyColor(node.latency)}`}>
                  {node.latency}ms
                </span>
                <span className="text-slate-500">丢包 {node.packetLoss}%</span>
              </div>
            </motion.div>
          ))}
        </div>
